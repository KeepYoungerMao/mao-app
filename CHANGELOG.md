...
- 新增用户关联部门API接口；
- 新增部门相关API；
- 新增数据状态枚举映射数据库中状态类字段，可通用；
- 新增部门类型枚举，严格定义部门等级逻辑；
- 新增表`sys_employee_code_sequence`用于员工编号序列号管理；
- 实现员工编号生成器，由{2位年份}+{6位部门编号}+{4位序列号}组成；
...
- 对字典缓存逻辑进行重新规划；
- 省市区表重命名为：`sys_region`；
- 用户资料表，更名为员工表，教育经历、工作经历、人员关系、材料信息表更改为与员工表相关联，不与用户表关联；
- 部门表现在与员工表关联，不与用户表关联；
- 表结构更改后相关的API逻辑更新；
...
- 新增用户资料教育经历、工作经历、人员关系和上传材料的查询、新增、更新、删除接口；
...
- 删除用户资料-教育经历、工作经历、人员关系表中的order字段；
- 完善删除用户接口，删除用户时用户资料所有数据都将删除；

...
import com.mao.metric.entity.MinuteServerMetricSnapshot
import com.mao.metric.repository.ServerMetricRepository
...
class ScheduleConfiguration(
    ...
    fun persistCurrentMinuteServerMetrics() = runBlocking {
        ...
        try {
            ...
            // 取老桶
            val oldBucket = serverService.rotateMinuteBucket()
            // 获取桶里数据
    ...
    private suspend fun saveMinuteServerMetricSnapshot(snapshot: MinuteServerMetricSnapshot) {
        ...
        serverMetricRepository.save(
            ServerMetric(
                ...
                onlineUsers = serverService.getCurrentOnlineUser(),
                loginUsers = serverService.getTodayLoginUserCount(),
                createdTime = now,
                isNewRecord = true
            )
        )
